...
def main(args, attackLM, targetLM, judgeLM):
    ...
    for x in range(10):
        ...
        convs_list = [conv_template(attackLM.template)
        ...
        for iteration in range(1, args.n_iterations + 1):
            print(f"""\n{'='*36}\nIteration: {iteration}\n{'='*36}\n""")
            if iteration > 1:
                processed_response_list = [process_target_response(
                    target_response, score, args.goal, args.target_str) for target_response, score in zip(target_response_list, judge_scores)]
            ...
            adv_prompt_list = [attack["prompt"]
                               for attack in extracted_attack_list]
            improv_list = [attack["improvement"]
                           for attack in extracted_attack_list]
...
if __name__ == '__main__':
    ...
    df = read_excel('data/results_vicunavsvicuna_selfreminder_50.csv')
    args.project_name = "a4-vicunavsvicuna-selfreminder-mentalhealth"  # WandB

    attackLM, targetLM = load_attack_and_target_models(args)

    # Iterate over each row in the DataFrame
    for index, row in df.iterrows():
        if row['is_jailbroken']:
            # Update args with values from the current row
            args.target_str = row['target_str']
            args.goal = row['goal']
            args.category = row['category']

            print('target: ' + args.target_str)
